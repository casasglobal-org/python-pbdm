from psymple_connections.connections.connection_ported_objects import CompositePortedObjectWithConnections, PortedObjectWithAssignmentsAndConnections
...
class PopulationObject(
    PortedObjectWithSearch,
    ParameterSearchObject
):
    PORTED_OBJECT_DATA = {
        "name", "input_ports", "output_ports", "variable_ports", "inputs", "output_connections", "variable_connections", "parsing_locals", 
    }
                ...
                if input in self.parameters:
                    ...
                    if input in self.input_ports:
                        ...
                        if value:=port.default_value:
                            ...
                            raise Exception(f"Parameter '{input}' has already been defined as an "
                                            f"input port of {self.address} with value {value} and "
                                            f"cannot be overwitten.")
                        else:
                            port.default_value = parameter_value
                    else:
                        self.add_input_ports({"name": input, "default_value": self.parameters[input]})
                else:
...
class CompositePopulationObject(
    ...
    PORTED_OBJECT_DATA = {
        "name", "children", "input_ports", "output_ports", "variable_ports", "inputs", "output_connections", "variable_connections", "variable_wires", "directed_wires", "parsing_locals", 
    }
...
class FunctionalPopulationObject(
    ...
    PORTED_OBJECT_DATA = {
        "name", "input_ports", "assignments", "create_input_ports", "inputs", "output_connections", "variable_connections", "parsing_locals", 
    }
...
class VariablePopulationObject(
    ...
    PORTED_OBJECT_DATA = {
        "name", "input_ports", "output_ports", "assignments", "create_input_ports", "create_output_ports", "inputs", "output_connections", "variable_connections", "parsing_locals", 
    }
